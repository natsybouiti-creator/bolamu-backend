...
Bolamu propose aujourd'hui à ses patients 4 mini-jeux de hasard/connaissance (grattage, roue, coffre, quiz) qui distribuent des Zora Points via le pipeline `awardZora()` déjà documenté dans `ARCHITECTURE_ZORA_BOLAMU.md`. Ces jeux existent pour créer une raison de revenir quotidiennement sur l'app (1 partie gratuite par jeu par jour) et pour gamifier l'acquisition de connaissances santé (quiz). Trois jeux supplémentaires (Bingo Santé, Parcours Elonga, Album de Masques) sont **teasés visuellement** au patient avec un badge « Bientôt » — la section 3 documente avec preuve qu'il n'existe **aucune trace backend, base de données, ou migration** pour ces trois jeux : intention produit à 100%, jamais commencée techniquement.
...
**Aucun mécanisme de déblocage séquentiel entre jeux n'existe côté backend.** Recherche exhaustive (`grep -i "unlock"` sur tout `src/`) : zéro occurrence. Les 4 jeux actifs (`scratch`/`wheel`/`chest`/`quiz`) sont accessibles indépendamment les uns des autres, sans condition de complétion préalable, dès que le patient est authentifié — confirmé par lecture de `zora-games.routes.js` (aucune vérification d'un jeu précédent dans `POST /games/play`) et de `zora-games.service.js` (`playGame()` ne consulte jamais l'historique d'un *autre* `game_type` que celui demandé).

### 3.1 Les 3 jeux « Bientôt » (Bingo Santé, Parcours Elonga, Album de Masques) — confirmé 100% non commencés côté backend

Recherche exhaustive le 17 juillet 2026 :
- **Base de données** : aucune ligne dans `zora_games` ne correspond à ces noms (les 4 seules lignes de la table sont `scratch`/`wheel`/`chest`/`quiz`, voir §2). Aucune autre table (`information_schema.tables`) ne porte un nom approchant.
- **Migrations** : `grep -ri "bingo\|parcours elonga\|album de masques"` sur tout `database/` → **0 résultat**.
- **Code source** : `grep -i "bingo\|unlock"` sur tout `src/` → **0 résultat**.
- **Frontend** : les 3 cartes existent bien, en dur, dans `public/patient/dashboard.html:1044-1046`, chacune avec un badge visuel statique `<span class="material-symbols-outlined">lock</span>Bientôt` (fond gris `rgba(67,70,84,0.08)`) — **aucun `onclick`, aucune fonction JS associée**, contrairement aux 4 jeux actifs qui ont chacun un `onclick="A.openGame('...')"`. Ce sont des `<div>` purement statiques, sans logique de progression, sans lien avec une quelconque donnée backend.
...
| Liste des 4 cartes de jeu | `A.renderGames()` (`:2429`), à partir de `A.gamesList` (statique, `:1674`) | Codé en dur — pas `GET /games/config` (voir §6.2) |
| 3 cartes "Bientôt" (Bingo Santé, Parcours Elonga, Album de Masques) | HTML statique (`:1044-1046`), aucun `onclick` | Aucune |
...
> ⚠️ **ROADMAP — NON IMPLÉMENTÉ.** Tout ce qui suit est un concept exploratoire. Aucune ligne de code, aucune table, aucune route n'existe pour ces jeux sauf mention explicite contraire (Bingo Santé / Parcours Elonga / Album de Masques : teasés visuellement côté frontend uniquement, §3.1 — statut "partiellement construit" au sens strict où un badge UI existe, mais 0% de logique ou de donnée réelle).
...
- **Nom et mécanique** : grille de cases santé (ex. "boire 2L d'eau", "10 000 pas", "check-in Elonga") cochées au fil de la semaine ; ligne/colonne complète = récompense.
- **Modèle** : solo (asynchrone, sur plusieurs jours).
- **Compatibilité `awardZora()`** : bonne — `action_type='game_bingo_line'`, `proof_reference` = ID de grille + numéro de ligne (naturellement unique), `proof_class='system_event'` si les cases sont validées par des événements système déjà tracés (ex. `zora_ledger` existant pour "10 000 pas" si ce capteur existe un jour), sinon `device_declared` (rejeté par le pipeline) si auto-déclaratif — point de vigilance à trancher avant implémentation.
- **Effort** : backend moyen (table grille + cases + état par patient, cron de génération hebdo) ; frontend moyen (grille interactive) ; contenu faible (réutilise des actions santé déjà suivies).
- **Statut** : partiellement construit — carte "Bientôt" visuelle statique existante (`dashboard.html:1044`), 0% de logique/donnée (§3.1).
...
| Terme | Statut | Source |
|---|---|---|
| **`zora_games`** | Réel | 4 lignes, `game_type` scratch/wheel/chest/quiz, §2 |
| **`zora_game_prizes`** | Réel | 15 lignes (5×3 jeux, pas de lots pour quiz), §2/§5 |
...
| **Bingo Santé / Parcours Elonga / Album de Masques** | Vision (badge visuel seul) | 0% backend, §3.1 |
