...
| 1.4 | 17 juillet 2026 (nuit, encore plus tard) | §2.5 : `daily_cap=1` de `zora_earn_rules.game_bingo` (copié par erreur du pattern des 4 autres jeux) retiré (`NULL`, `migration_085_fix_bingo_daily_cap.sql`) — bloquait des crédits Bingo légitimes le même jour (ex. bonus bingo complet perdu si une ligne avait déjà consommé le cap). Testé en conditions réelles : ligne + colonne + bingo complet (15+15+100 Zora) crédités séparément le même jour sans blocage. |
...
- **`daily_cap` retiré, résolu le 17 juillet 2026 (`migration_085_fix_bingo_daily_cap.sql`)** : `zora_earn_rules.game_bingo.daily_cap` valait initialement `1`, copié par erreur du pattern des 4 autres jeux (1 partie/jour = 1 crédit/jour) — pour le Bingo, qui n'a pas de notion de "partie", ce cap ne protégeait contre rien (l'idempotence réelle est garantie par `bingo_grids.lines_rewarded`/`bingo_rewarded`) et ne faisait que bloquer des crédits légitimes le même jour (ex. bonus bingo complet de 100 Zora non crédité si une ligne avait déjà consommé le cap). Mis à `NULL` (pas un plafond haut arbitraire) plutôt que retiré de `awardZora()` lui-même — `if (rule.daily_cap && ...)` désactive proprement la vérification pour cette règle sans toucher au comportement des 4 autres jeux. **Testé en conditions réelles le 17 juillet 2026** : ligne, colonne et bingo complet simulés le même jour → les 3 crédits (15 + 15 + 100 Zora) sont passés séparément dans `zora_ledger` en quelques secondes d'intervalle, sans aucun `daily_cap_reached`.
