...
- **⚠️ Point de vigilance sécurité** : `POST /grant` et `POST /distribute-monthly` ne sont protégées que par `authMiddleware` — **aucune vérification de rôle admin** dans le code de ces deux routes (contrairement à `payouts.routes.js`/`clearing.routes.js` qui ont un `adminOnly` explicite). N'importe quel utilisateur authentifié peut donc appeler ces endpoints. À signaler à `/security-officer`, sans correction ici.
