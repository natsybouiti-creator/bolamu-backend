...
- **✅ Corrigé** : `POST /grant`, `POST /distribute-monthly`, `GET /admin/all`, `POST /partners` et `PATCH /partners/:id` n'étaient protégées que par `authMiddleware`, sans vérification de rôle admin (contrairement à `payouts.routes.js`/`clearing.routes.js`, qui ont un `adminOnly` explicite) — n'importe quel utilisateur authentifié pouvait s'attribuer des crédits, déclencher une distribution mensuelle à tous les comptes éligibles, lister tous les comptes crédits, ou créer/modifier un partenaire crédits. Les 5 routes sont désormais protégées par le même middleware `adminOnly` que `payouts.routes.js`/`clearing.routes.js` (`src/routes/credits.routes.js`, commits `fix(security): controle role admin sur credits grant/distribute-monthly` puis `fix(security): controle role admin sur credits admin/all, partners create/update`). `GET /balance`, `GET /balance/:phone`, `POST /spend`, `GET /partners` (liste publique) restent inchangées et accessibles comme avant — vérifié par test. Aucun appel frontend confirmé n'utilise `GET /admin/all`/`POST /partners`/`PATCH /partners/:id` à ce jour (grep exhaustif sur `public/`) — correctif préventif, pas de régression fonctionnelle possible. **Note distincte** : `public/admin/dashboard.html:1441` octroie des crédits via un chemin différent, `POST /admin/credits/grant` (pas `POST /credits/grant`) — probablement une route équivalente montée ailleurs (`admin.routes.js` ?), non auditée dans cette passe, à vérifier séparément si elle a le même gap.
