...
async function registerPharmacie(req, res) {
    ...
    if (!phone || !name || !responsible_name || !rccm_number || !city) {
        return res.status(400).json({
            success: false,
            message: 'Champs obligatoires : phone, name, responsible_name, rccm_number, city'
        });
    }
    ...
    try {
            ...
            try {
                const uploadResult = await new Promise((resolve, reject) => {
                    const stream = cloudinary.uploader.upload_stream(
                        { folder: 'bolamu/pharmacies/documents', resource_type: 'auto' },
                ...
                console.error('[Cloudinary pharmacie]', e.message);
            }
        }
        ...
        if (existingUser.rows.length === 0) {
            const newUser = await client.query(
                `INSERT INTO users (phone, full_name, role, is_active) VALUES ($1, $2, 'pharmacie', TRUE) RETURNING id`,
                [phone, name]
            );
        ...
        const newPharma = await client.query(
            `INSERT INTO pharmacies
                (phone, user_id, name, responsible_name, rccm_number, autorisation_number,
            ...
            [
                phone, userId, name, responsible_name, rccm_number,
                autorisation_number || null, city, neighborhood || null,
                autoStatus, memberCode, documentUrl, documentPublicId, score,
                momo_number || phone
            ]
        );
        ...
        await client.query(
            `INSERT INTO audit_log (event_type, actor_phone, target_table, target_id, payload)
             VALUES ('pharmacie.registered', $1, 'pharmacies', $2, $3)`,
        ...
        return res.status(201).json({
            ...
            data: {
                phone: newPharma.rows[0].phone,
                name: newPharma.rows[0].name,
                status: newPharma.rows[0].status,
                member_code: newPharma.rows[0].member_code,
                trust_score: newPharma.rows[0].trust_score,
...
async function getPharmacieProfile(req, res) {
    ...
    try {
        const result = await pool.query(
            `SELECT id, name, phone, responsible_name, rccm_number, city, neighborhood,
                    status, member_code, trust_score, momo_number, is_active, created_at
             FROM pharmacies WHERE phone = $1`,
            [phone]
        );
...
async function updatePharmacieStatus(req, res) {
    ...
    try {
        const result = await pool.query(
            `UPDATE pharmacies SET status = $1, is_active = $2 WHERE id = $3 RETURNING *`,
        ...
        try {
            const msgs = {
                verified: `Bolamu : ${p.name} validée ! Code : ${p.member_code}. Connectez-vous.`,
                rejected: `Bolamu : Inscription ${p.name} non validée. Motif : ${reason || 'Dossier incomplet'}.`,
                suspended: `Bolamu : Compte ${p.name} suspendu. Motif : ${reason || 'Activité suspecte'}.`
            };
