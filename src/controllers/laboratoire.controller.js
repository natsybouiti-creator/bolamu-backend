...
async function registerLaboratoire(req, res) {
    ...
    if (!phone || !name || !director_name || !rccm_number || !city) {
        return res.status(400).json({
            success: false,
            message: 'Champs obligatoires : phone, name, director_name, rccm_number, city'
        });
    }
    ...
    try {
            ...
            try {
                const uploadResult = await new Promise((resolve, reject) => {
                    const stream = cloudinary.uploader.upload_stream(
                        { folder: 'bolamu/laboratories/documents', resource_type: 'auto' },
                ...
                console.error('[Cloudinary labo]', e.message);
            }
        }
        ...
        if (existingUser.rows.length === 0) {
            const newUser = await client.query(
                `INSERT INTO users (phone, full_name, role, is_active) VALUES ($1, $2, 'laboratoire', TRUE) RETURNING id`,
                [phone, name]
            );
        ...
        const newLab = await client.query(
            `INSERT INTO laboratories
                (phone, user_id, name, director_name, rccm_number, agrement_number,
            ...
            [
                phone, userId, name, director_name, rccm_number,
                agrement_number || null, city, neighborhood || null,
        ...
        await client.query(
            `INSERT INTO audit_log (event_type, actor_phone, target_table, target_id, payload)
             VALUES ('laboratoire.registered', $1, 'laboratories', $2, $3)`,
        ...
        return res.status(201).json({
            ...
            data: {
                phone: newLab.rows[0].phone,
                name: newLab.rows[0].name,
                status: newLab.rows[0].status,
                member_code: newLab.rows[0].member_code,
                trust_score: newLab.rows[0].trust_score,
...
async function getLaboratoireProfile(req, res) {
    ...
    try {
        const result = await pool.query(
            `SELECT id, name, phone, director_name, rccm_number, agrement_number,
                    city, neighborhood, status, member_code, trust_score, momo_number, created_at
             FROM laboratories WHERE phone = $1`,
            [phone]
        );
...
async function updateLaboratoireStatus(req, res) {
    ...
    try {
        const result = await pool.query(
            `UPDATE laboratories SET status = $1, is_active = $2 WHERE id = $3 RETURNING *`,
        ...
        try {
            const msgs = {
                verified: `Bolamu : ${l.name} validé ! Code : ${l.member_code}. Connectez-vous.`,
                rejected: `Bolamu : Inscription ${l.name} non validée. Motif : ${reason || 'Dossier incomplet'}.`,
                suspended: `Bolamu : Compte ${l.name} suspendu. Motif : ${reason || 'Activité suspecte'}.`
            };
