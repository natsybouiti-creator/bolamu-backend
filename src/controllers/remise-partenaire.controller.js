...
// ─── INITIER UNE TRANSACTION REMISE PARTENAIRE ──────────────────────────────────
async function initiateTransaction(req, res) {
    ...
    if (partnerRole !== 'pharmacie' && partnerRole !== 'laboratoire') {
        return res.status(403).json({ success: false, message: 'Seuls pharmacie et laboratoire peuvent initier des transactions de remise partenaire.' });
    }
    ...
    try {
        ...
        // 5. INSERT dans transactions_remise_partenaire
        const insertRes = await client.query(
            `INSERT INTO transactions_remise_partenaire 
        ...
        await client.query(
            `INSERT INTO audit_log (event_type, actor_phone, target_table, target_id, payload)
             VALUES ('REMISE_PARTENAIRE_INITIATED', $1, 'transactions_remise_partenaire', $2, $3::jsonb)`,
...
async function validateTransaction(req, res) {
    ...
    try {
        ...
        const transCheck = await client.query(
            `SELECT * FROM transactions_remise_partenaire 
        ...
        const updateRes = await client.query(
            `UPDATE transactions_remise_partenaire 
        ...
        await client.query(
            `INSERT INTO audit_log (event_type, actor_phone, target_table, target_id, payload)
             VALUES ('REMISE_PARTENAIRE_VALIDATED', $1, 'transactions_remise_partenaire', $2, $3::jsonb)`,
...
async function listTransactionsPartenaire(req, res) {
    ...
    try {
        let query = `
            ...
            FROM transactions_remise_partenaire ttp
        ...
        // Compte total
        let countQuery = `SELECT COUNT(*) FROM transactions_remise_partenaire WHERE partner_phone = $1`;
...
async function listTransactionsAdmin(req, res) {
    ...
    try {
        ...
        // Compte total
        let countQuery = `
            SELECT COUNT(*) FROM transactions_remise_partenaire
            WHERE 1=1
        `;
        const countParams = [];
...
async function reconcileTransaction(req, res) {
    ...
    try {
        ...
        const transCheck = await client.query(
            `SELECT * FROM transactions_remise_partenaire WHERE id = $1`,
            [id]
        );
        ...
        await client.query(
            `INSERT INTO audit_log (event_type, actor_phone, target_table, target_id, payload)
             VALUES ('REMISE_PARTENAIRE_RECONCILED', $1, 'transactions_remise_partenaire', $2, $3::jsonb)`,
