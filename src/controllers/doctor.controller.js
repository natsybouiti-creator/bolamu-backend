...
function calculateTrustScore(data) {
    ...
    if (/^\+242(06|05)[0-9]{7}$/.test(data.phone)) {
        score += 20; details.push({ check: 'phone_valid', points: 20 });
    }
    if (data.registration_number && /^[A-Z0-9\-\/]{4,20}$/i.test(data.registration_number)) {
        score += 25; details.push({ check: 'registration_number_format', points: 25 });
    }
    if (data.document_url) {
        score += 30; details.push({ check: 'document_uploaded', points: 30 });
    }
    if (data.full_name && data.full_name.trim().split(' ').length >= 2) {
        score += 10; details.push({ check: 'full_name_valid', points: 10 });
    }
    if (data.specialty && data.specialty.length > 3) {
        score += 10; details.push({ check: 'specialty_valid', points: 10 });
    }
    if (data.city) {
        score += 5; details.push({ check: 'city_valid', points: 5 });
    }
...
async function registerDoctor(req, res) {
    ...
    if (!/^\+242[0-9]{9}$/.test(phone)) {
        return res.status(400).json({
            success: false,
            message: 'Numéro invalide. Format : +242XXXXXXXXX'
        });
    }
    ...
    try {
        ...
        // Upload Cloudinary si document présent
            ...
            try {
                const uploadResult = await new Promise((resolve, reject) => {
                    const stream = cloudinary.uploader.upload_stream(
                        { folder: 'bolamu/doctors/documents', resource_type: 'auto' },
        ...
        const { score, details } = calculateTrustScore({
            phone, full_name, specialty, registration_number, city, document_url: documentUrl
        });
        ...
        const newDoctor = await client.query(
            ...
            [
                phone, userId, full_name, specialty, registration_number,
                city, neighborhood || null, bio || null,
                autoStatus, memberCode,
                documentUrl, documentPublicId, score,
                momo_number || phone
            ]
        );
        ...
        try {
            ...
            console.log('⚠️ SMS non envoyé:', smsErr.message);
        }
        ...
        return res.status(201).json({
            ...
            message: autoStatus === 'verified'
                ...
                : '📋 Inscription reçue. Vérification sous 24h.',
            data: {
                phone: newDoctor.rows[0].phone,
                full_name: newDoctor.rows[0].full_name,
                status: newDoctor.rows[0].status,
                member_code: newDoctor.rows[0].member_code,
                trust_score: newDoctor.rows[0].trust_score,
                auto_validated: autoStatus === 'verified'
            }
        });
...
async function getDoctors(req, res) {
    ...
    try {
        ...
        const result = await pool.query(
            `SELECT d.id, d.full_name, d.specialty, d.city, d.neighborhood,
                    d.bio, d.availability_schedule, d.total_consultations, d.member_code
             FROM doctors d WHERE ${whereClause}
...
async function updateDoctorStatus(req, res) {
    ...
    try {
        const result = await pool.query(
            `UPDATE doctors SET status = $1, is_active = $2 WHERE id = $3 RETURNING *`,
        ...
        console.error('[updateDoctorStatus]', error.message);
        return res.status(500).json({ success: false, message: 'Erreur serveur.' });
    }
}
