...
// ── MIDDLEWARE : accepte admin ET content_admin ──────────────────────────────
// Utilisé sur toutes les routes /admin/* de ce fichier
function requireContentAdmin(req, res, next) {
  const allowed = ['admin', 'content_admin'];
  if (!req.user || !allowed.includes(req.user.role)) {
    return res.status(403).json({ success: false, message: 'Accès réservé aux admins éditoriaux.' });
  }
  next();
}
...
// GET /api/v1/articles/content-blocks — blocs publics (pour index.html)
...
// ════════════════════════════════════════════════════════════════════════════
//  ADMIN — articles (admin + content_admin)
// ════════════════════════════════════════════════════════════════════════════
...
// GET /api/v1/articles/admin/content-blocks
router.get('/admin/content-blocks', authMiddleware, requireContentAdmin, async (req, res) => {
  try {
...
// PUT /api/v1/articles/admin/content-blocks/:key
router.put('/admin/content-blocks/:key', authMiddleware, requireContentAdmin, async (req, res) => {
  try {
...
// POST /api/v1/articles/admin/upload-vitrine
router.post('/admin/upload-vitrine', authMiddleware, requireContentAdmin, upload.single('image'), async (req, res) => {
  try {
...
router.post('/upload-image', authMiddleware, requireContentAdmin, upload.single('image'), async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ success: false, message: 'Aucune image' });
...
router.post('/', authMiddleware, requireContentAdmin, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [title, excerpt||'', content||'', category||'prevention', author||'Dr. Bolamu',
       read_time||'5 min', emoji||'📄', image_url||'', is_published||false, is_featured||false]
    );
    await pool.query(
      `INSERT INTO audit_log (event_type, actor_phone, target_table, target_id, payload)
...
router.put('/:id', authMiddleware, requireContentAdmin, async (req, res) => {
  try {
    const fields = ['title','excerpt','content','category','author','read_time','emoji','image_url','is_published','is_featured'];
    const updates = []; const vals = [];
    fields.forEach(f => {
      if (req.body[f] !== undefined) { updates.push(`${f}=$${vals.length+1}`); vals.push(req.body[f]); }
    });
    if (!updates.length) return res.status(400).json({ success: false, message: 'Rien à mettre à jour' });
    updates.push(`updated_at=NOW()`);
    vals.push(req.params.id);
    const { rows } = await pool.query(
      `UPDATE articles SET ${updates.join(',')} WHERE id=$${vals.length} RETURNING *`, vals
    );
...
router.delete('/:id', authMiddleware, requireContentAdmin, async (req, res) => {
  try {
...
// GET /api/v1/articles/:id — article unique public (en dernier pour ne pas intercepter /admin/*)
