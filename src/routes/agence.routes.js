...
router.post('/import-employes', requireAgent, async (req, res) => {
  const client = await pool.connect();
  try {
    const { employes, contract_id } = req.body;
    ...
    for (const emp of employes) {
      ...
      // contract_id peut venir du corps de la requête (niveau lot) ou de chaque employé
      ...
      try {
        ...
        // Lier l'employé au contrat si contract_id fourni (colonnes réelles
        // de company_employees — avant : company_contract_id/company_id/
        // patient_phone/enrolled_at/enrolled_by, aucune n'existe sur la table)
        if (contractId) {
          await client.query(
            `INSERT INTO company_employees
               (contract_id, employee_phone, employee_name, subscription_id, categorie_rh, status)
             VALUES ($1, $2, $3, $4, $5, 'active')
             ON CONFLICT (contract_id, employee_phone) DO UPDATE
               SET subscription_id = EXCLUDED.subscription_id, employee_name = EXCLUDED.employee_name, updated_at = NOW()`,
            [contractId, phone, `${prenom} ${nom}`.trim(), sub.rows[0].id, categorie_rh || null]
          );
        }
        ...
        await client.query(
          ...
          [req.user.phone, sub.rows[0].id, JSON.stringify({ patient_phone: phone, plan: planEnum, amount, payment_method, contract_id: contractId })]
        );
