...
**⚠️ Contradiction interne entre documents canoniques** : `CLAUDE.md` (`/ui-designer`) liste explicitement `#2E86FF` comme couleur **interdite** (« n'existe pas dans la charte »), alors que `docs/design_system.md` §12 la définit officiellement comme `--role-medecin`. Les deux fichiers sont internes au projet et se contredisent directement sur ce point précis — à faire trancher par `/ui-designer` (probable oubli de mise à jour de `CLAUDE.md` lors de l'introduction des couleurs de rôle en §12 de `design_system.md`, ou l'inverse). Non tranché ici.
