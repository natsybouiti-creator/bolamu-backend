...
- Visualisation document admin (storage_path direct) ✅
- Dashboard patient : token sauvegardé, abonnement actif ✅
- Dossier médical patient (constantes POST) ✅
...
- PWA : manifest.json + favicons tous navigateurs ✅
...
- bolamu_patient_token non sauvegardé après login — corrigé : frontend lit le bon champ accessToken
- loadConflits lisait d.conflicts au lieu de d.data.conflicts — corrigé
- conflict_actions INSERT échouait silencieusement (audit_log payload text vs jsonb) — corrigé : cast $5::jsonb
- paramCount non incrémenté dans transitionStatut — corrigé : utilise values.length
- Enum subscription_plan : bronze/silver/gold inexistants — valeurs réelles : essentiel/standard/premium
- Enum canal_paiement_enum : valeurs réelles : ovp_bancaire/momo_annuel/familial/sepa_diaspora
...
### Corrections appliquées — 30 mai 2026
✅ Module conflits complet : soumission patient, historique, gestion admin (statut, escalade, messages)
✅ BHP v1.2 : health_records + consentements + access_log + purge cron
✅ Cloudinary authenticated : upload + visualisation admin sécurisée
✅ Landing page Next.js statique : public/index.html
✅ PWA complète : manifest.json + apple-touch-icon + favicon tous navigateurs
✅ Dashboards RH et Secrétaire : créés et montés
✅ Register multi-étapes : uploads par rôle (JSONB)
✅ Redis/BullMQ : rendu optionnel (ne crashe plus les routes critiques)
✅ Panels admin Collecte/Conventions/Clearing CDR : débloqués (fix go())
...
- Patient test RDV : +242065458932 (RifRaf Jordan — abonnement essentiel actif via SQL, inscription subscriptions)
...
### EN COURS DE TEST — 30 mai 2026
- Dashboard patient : prise de RDV (en cours de validation 30 mai)
- Dashboard médecin : à valider
- Dashboard pharmacie : à valider
- Dashboard laboratoire : à valider
- Actions conflits admin : changement statut validé, assignation à compléter
- Collecte admin : KPIs dashboard à vérifier
...
## NOUVELLES ROUTES MONTÉES — 30 MAI 2026
...
- /api/v1/admin/documents/* : visualisation document admin (storage_path direct)
- /api/v1/health-records/* : BHP v1.2 health_records
- /api/v1/consent/* : BHP v1.2 consentements
- /api/v1/upload/* : upload Cloudinary authenticated
