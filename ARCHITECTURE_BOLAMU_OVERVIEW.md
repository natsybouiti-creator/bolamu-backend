...
- ✅ **Corrigé (Passe 3, commits `41e8d23`/`3c8a525`/`6354319`)** — ~~Deux systèmes d'ordonnance parallèles non unifiés~~ : `prescriptions` retenue comme système canonique (seule à avoir des preuves réelles de dispensation de bout en bout) — `medecin/dashboard.html` appelle désormais `POST /prescriptions/create` au lieu de `POST /ordonnances` ; `ordonnances`/`ordonnance_items` dépréciées (commentées, non supprimées, données existantes intactes). Gap financier découvert et corrigé au passage : la dispensation active (`deliverPrescription()`) ne générait **aucun** `clearing_transactions` — corrigé, génère désormais un versement CDR pharmacie (tarif `platform_config.tarif_clearing_pharmacie`, migration_060) + notifie le médecin en plus du patient. `patient.routes.js::GET /consultations/recentes` redirigé vers `prescriptions` en conséquence.
- ✅ **Corrigé (Passe 2, commit `9af55a8`)** — ~~Traçabilité agent à moitié cassée~~ : les 3 points d'écriture directe d'`agence.routes.js` (`/souscrire`, `/souscrire-complet`, `/import-employes`) renseignent désormais `agent_phone`, testé contre Neon (une inscription via ce portail remonte bien dans `GET /agent/mes-inscrits`). Décision produit associée : `agent/dashboard.html` reste le portail officiel, `agence/dashboard.html` coexiste pour ses fonctionnalités propres (wizard, réclamations, annuaire) avec traçabilité désormais unifiée — pas de fusion des deux portails.
...
- ✅ **Corrigé (Passe 2, commit `f5a2b0c`)** — ~~Incohérence `content_admin`~~ : périmètre élargi par décision produit à `GET /admin/stats` et `GET /admin/users` uniquement (middleware `adminOrContentAdmin`), toutes les autres routes d'`admin.routes.js` (financières, modération, config) restent `adminOnly` strict. `admin/content.html` complété avec les 2 sections correspondantes (lecture seule).
...
- ✅ **Corrigé (Passe 2, commit `3353a99`)** — ~~`ARCHITECTURE_ZORA_BOLAMU.md` décrit un schéma qui ne correspond pas à la production~~ : avertissement de statut (« SPÉCIFICATION CIBLE ») ajouté en tête de document, renvoyant vers le schéma réel (§3.3 de ce document, `ARCHITECTURE_MODELE_DONNEES_BOLAMU.md` §1.5).
- ✅ **Corrigé (Passe 2, commit `3353a99`)** — ~~`PILOTAGE_BOLAMU.md` contient des affirmations obsolètes~~ : avertissement « DOCUMENT HISTORIQUE » ajouté en tête, signalant explicitement l'erreur `wame.service.js` et renvoyant vers ce document pour l'état actuel.
...
- **Bug FK `consultations.rdv_id` → `rendez_vous`** (découvert en Passe 3, non corrigé — scope trop large) : `medecin/dashboard.html` envoie en réalité un `appointments.id` comme `rdv_id`, alors que la contrainte FK réelle référence `rendez_vous` (table à 4 lignes, quasi vide). La contrainte n'est satisfaite que par coïncidence numérique entre les deux séquences `SERIAL` — `POST /consultations/open` échoue avec une violation FK dès que l'`appointments.id` ne coïncide pas avec une ligne `rendez_vous` existante. Documenté en détail dans `ARCHITECTURE_SOINS_BOLAMU.md` §1 ; le pont `patient.routes.js::GET /consultations/recentes` (§3) hérite de cette même fragilité.
